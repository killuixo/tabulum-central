...
// Leitor seguro de Variáveis de Ambiente (Vite)
const getEnv = (key) => {
...
const URL_DADOS_GERAIS = getEnv('VITE_DADOS_GERAIS_URL'); // Traz Estado e Capital juntos
...
// Extrator Inteligente de Tema (mesma lógica da sua planilha)
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                // Fallback Silencioso para Mock (Para você visualizar o design se o Vercel não tiver as variáveis ainda)
                ...
                // Processamento e Normalização das Chaves
...
const NativeBarChart = ({ data, colorClass, maxItems = 5 }) => {
...
const Sidebar = () => {
    ...
    return (
        <div className="w-80 bg-white border-r-4 border-black flex flex-col z-20 flex-shrink-0 h-full">
                            ...
                            <div className="max-h-40 overflow-y-auto border-2 border-black bg-white p-2 space-y-1">
                                {regioes.map(r => (
                                    <label key={r} className="flex items-center space-x-2 cursor-pointer p-1.5 hover:bg-gray-100 group">
                                        ...
                                        <span className="text-xs font-bold uppercase truncate group-hover:text-[#007D8A]">{r}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="max-h-40 overflow-y-auto border-2 border-black bg-white p-2 space-y-1">
                                {temas.map(t => (
                                    <label key={t} className="flex items-center space-x-2 cursor-pointer p-1.5 hover:bg-gray-100 group">
                                        ...
                                        <span className="text-xs font-bold uppercase truncate group-hover:text-[#EAA221]">{t}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
        ...
        <div className="space-y-8 w-full max-w-6xl mx-auto">
            {isMock && (
                <div className="bg-black text-white p-4 font-black uppercase text-xs flex items-center border-4 border-[#EAA221] shadow-[4px_4px_0_0_#EAA221]">
                    ⚠️ Demonstração visual. Configure as Variáveis de Ambiente no Vercel (VITE_LEADS_URL, etc) para ver dados reais.
                </div>
            )}
            
            <div className="border-b-4 border-black pb-4">
                <h2 className="text-3xl font-black uppercase tracking-tight text-black">Visão Global</h2>
                <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-1">Sistematização de Dados Cruzados</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0_0_rgba(17,17,17,1)]">
                    ...
                    <div className="h-2 w-full bg-[#C1272D] border-2 border-black mt-4"></div>
                </div>
                <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0_0_rgba(17,17,17,1)]">
                    ...
                    <div className="h-2 w-full bg-[#EAA221] border-2 border-black mt-4"></div>
                </div>
                <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0_0_rgba(17,17,17,1)]">
                    ...
                    <div className="h-2 w-full bg-[#007D8A] border-2 border-black mt-4"></div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
                <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0_0_rgba(17,17,17,1)] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <NativeBarChart data={chartDataTemas} colorClass="bg-[#EAA221]" maxItems={6} />
                    </div>
                </div>
                ...
                <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0_0_rgba(17,17,17,1)] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <NativeBarChart data={chartDataLocais} colorClass="bg-[#007D8A]" maxItems={6} />
                    </div>
                </div>
            </div>
        </div>
...
const FichaCompleta = () => {
    ...
    const entityData = useMemo(() => {
        ...
        if (type === 'municipio') {
            ...
            // Busca na aba ESTADO
            const v = estado.find(r => String(r['Cidade'] || r['cidade'] || r['Município'] || '').toLowerCase() === n);
    ...
    return (
        <div className="space-y-6 w-full max-w-5xl mx-auto pb-12">
            <button 
                ...
                className="bg-black text-white px-4 py-2 font-black uppercase text-[10px] border-4 border-black hover:bg-gray-800 flex items-center w-fit shadow-[4px_4px_0_0_rgba(17,17,17,1)]"
            >
            ...
            <div className="bg-white p-8 border-4 border-black shadow-[8px_8px_0_0_rgba(17,17,17,1)] relative overflow-hidden">
                ...
                <h1 className="text-4xl md:text-5xl font-black text-black uppercase tracking-tighter leading-none">{selectedEntity.name}</h1>
                ...
                {entityData.votos && (
                    <div className="mt-6 flex flex-wrap gap-4">
                        <div className="bg-gray-100 border-2 border-black px-4 py-2">
                            ...
                            <span className="text-xl font-black text-[#C1272D]">{entityData.votos.votos}</span>
                        </div>
                        <div className="bg-gray-100 border-2 border-black px-4 py-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white border-4 border-black shadow-[6px_6px_0_0_rgba(17,17,17,1)] flex flex-col">
                    ...
                    <div className="p-4 bg-gray-50 border-b-4 border-black">
                        ...
                        <span className="text-2xl font-black text-[#007D8A]">{formatCurrency(valTotal)}</span>
                    </div>
                            ...
                            <table className="w-full text-left">
                                <tbody>
                                    {entityData.emendas.map((e, i) => (
                                        <tr key={i} className="border-b-2 border-gray-200 hover:bg-gray-100">
                                            <td className="p-3">
                                            ...
                                            <td className="p-3 text-right whitespace-nowrap font-black text-sm text-[#007D8A]">
                ...
                <div className="bg-white border-4 border-black shadow-[6px_6px_0_0_rgba(17,17,17,1)] flex flex-col">
                    ...
                    <div className="overflow-x-auto max-h-[470px]">
                        ...
                        ) : <div className="p-8 text-center text-gray-400 font-bold uppercase text-sm">Nenhum lead encontrado.</div>}
                    </div>
                </div>
            </div>
        </div>
...
const AppContent = () => {
    ...
    if (loading) return (
        <div className="h-screen w-screen flex flex-col items-center justify-center bg-[#FDFBF7]">
            <div className="w-16 h-16 border-4 border-[#FDFBF7] border-t-[#C1272D] border-r-[#EAA221] border-b-[#007D8A] rounded-full animate-spin mb-6"></div>
            <p className="text-sm font-black tracking-widest uppercase border-4 border-black bg-white px-4 py-2 shadow-[4px_4px_0_0_rgba(17,17,17,1)]">Sincronizando Base...</p>
        </div>
    );

    return (
        <div className="flex h-screen w-full overflow-hidden bg-[#FDFBF7] text-[#111111] font-sans">
            <Sidebar />
