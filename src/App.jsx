...
// Extrator robusto de colunas que ignora acentos, maiúsculas ou minúsculas
const getField = (obj, possibleKeys) => {
    const keys = Object.keys(obj);
    for (const pk of possibleKeys) {
        const match = keys.find(k => normalizeStr(k) === normalizeStr(pk));
...
const Icons = {
    ...
    Download: () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>,
    Printer: () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>
};
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const leads = leadsRaw.map((l, i) => {
                    const mun = getField(l, ['CIDADE', 'cidade']).trim();
                    const b = getField(l, ['BAIRRO REVISADO + REPLAN', 'bairroReplan', 'bairro']).trim();
                    return {
                        id: `l_${i}`,
                        nome: getField(l, ['NOME', 'nome']) || 'Anônimo',
                        municipio: mun,
                        bairro: b,
                        ...
                        tema: getTemaFromOrigem(getField(l, ['ORIGEM', 'origem']))
                    };
                }).filter(l => l.municipio);

                const emendas = emendasRaw.map((e, i) => {
                    const mun = getField(e, ['MUNICÍPIO', 'municipio']).trim();
                    const razaoSocialRaw = getField(e, ['RAZÃO SOCIAL', 'razao social']).trim();
                    const esfera = getField(e, ['ESFERA DE APLICAÇÃO', 'esfera de aplicação']).trim();
                    ...
                    return {
                        id: `e_${i}`,
                        numero: getField(e, ['NÚMERO DA EMENDA', 'numero']),
                        municipio: mun,
                        bairro: null,
                        regiao: (getField(e, ['REGIÃO', 'regiao']) || muniToRegiao[normalizeStr(mun)] || '').trim(),
                        distrito: null,
                        tema: getField(e, ['TEMA', 'tema']),
                        objeto: getField(e, ['OBJETO', 'objeto']),
                        total: parseCurrency(getField(e, ['TOTAL', 'total'])),
                        articulador: getField(e, ['ARTICULADOR', 'articulador']).trim(),
                        razaoSocial: razaoSocialFinal
                    };
                }).filter(e => e.numero);

                const agenda = agendaRaw.map((a, i) => {
                    let mun = getField(a, ['MUNICÍPIO', 'municipio', 'Município']).trim();
                    const b = getField(a, ['BAIRRO', 'bairro', 'Bairro']).trim();
                    const local = getField(a, ['LOCAL', 'local', 'Local']).trim();
                    ...
                    return {
                        id: `a_${i}`,
                        titulo: getField(a, ['TÍTULO', 'titulo', 'Título']),
                        municipio: mun,
                        bairro: b,
                        regiao: isFloripa(mun) ? bairroToRegiao[normalizeStr(b)] : muniToRegiao[normalizeStr(mun)],
                        distrito: isFloripa(mun) ? bairroToDistrito[normalizeStr(b)] : null,
                        tema: '',
                        articulador: getField(a, ['ARTICULADOR', 'articulador', 'Articulador']).trim(),
                        inicio: getField(a, ['INÍCIO', 'inicio', 'Início']) || null
                    };
                });

                const contatos = contatosRaw.map((c, i) => {
                    const munBairro = getField(c, ['municipio_bairro', 'MUNICÍPIO', 'Bairro REPLAN']).trim();
                    const isF = getField(c, ['base', 'BASE']).includes('Florianópolis');
                    
                    return {
                        id: `c_${i}`,
                        nome: getField(c, ['lideranca', 'LIDERANÇA']),
                        base: getField(c, ['base', 'BASE']),
                        municipio: isF ? 'Florianópolis' : munBairro,
                        bairro: isF ? munBairro : null,
                        regiao: getField(c, ['regiao', 'REGIÃO']).trim(),
                        distrito: getField(c, ['distrito', 'DISTRITO']).trim(),
                        tema: getField(c, ['temas', 'TEMAS']),
                        situacao: getField(c, ['situacao', 'SITUAÇÃO']),
                        articulador: getField(c, ['articulador', 'ARTICULADOR'])
                    };
                }).filter(c => c.nome);
...
const Sidebar = () => {
    ...
    // Usando rawData para garantir que as opções da sidebar não sumam ao marcar outros filtros
...
const DashPanel = ({ title, icon: Icon, colorClass, data, metricType, territoryScope, hasTema = true, onLabelClick }) => {
    ...
    const aggregated = useMemo(() => {
        ...
        data.forEach(item => {
            let key = dim === 'municipio' ? item.municipio :
    ...
    return (
        <div className="space-y-6 animate-fade-in w-full max-w-6xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 no-print">
                <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter">Raio-X: Estado</h2>
                <div className="flex flex-wrap gap-2">
                    <button onClick={() => window.print()} className="bg-black text-white px-4 py-2 font-black uppercase text-[10px] md:text-xs border-4 border-black hover:bg-gray-800 flex items-center shadow-[4px_4px_0_0_rgba(17,17,17,1)] active:translate-y-1 active:shadow-none transition-all">
                        <Icons.Printer /> <span className="ml-2">Exportar PDF</span>
                    </button>
                    <button onClick={() => exportToCSV(items, 'tabulum_municipios')} className="bg-[#EAA221] text-black px-4 py-2 font-black uppercase text-[10px] md:text-xs border-4 border-black hover:bg-yellow-500 flex items-center shadow-[4px_4px_0_0_rgba(17,17,17,1)] active:translate-y-1 active:shadow-none transition-all">
                        <Icons.Download /> <span className="ml-2">Exportar CSV</span>
                    </button>
                </div>
            </div>

            <div className="bg-white border-4 border-black shadow-[6px_6px_0px_0px_#111111] overflow-x-auto print:shadow-none print:overflow-visible">
                <table className="w-full text-left border-collapse min-w-[800px] print:min-w-full">
                    ...
                    <tbody>
                        {items.map((m, i) => (
                            <tr key={i} className="border-b-2 border-gray-200 hover:bg-[#EAA221]/20 transition-colors page-break-inside-avoid">
...
const ListaCapital = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in w-full max-w-6xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 no-print">
                <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter">Raio-X: Capital</h2>
                <div className="flex flex-wrap gap-2">
                    <button onClick={() => window.print()} className="bg-black text-white px-4 py-2 font-black uppercase text-[10px] md:text-xs border-4 border-black hover:bg-gray-800 flex items-center shadow-[4px_4px_0_0_rgba(17,17,17,1)] active:translate-y-1 active:shadow-none transition-all">
                        <Icons.Printer /> <span className="ml-2">Exportar PDF</span>
                    </button>
                    <button onClick={() => exportToCSV(items, 'tabulum_capital')} className="bg-[#007D8A] text-white px-4 py-2 font-black uppercase text-[10px] md:text-xs border-4 border-black hover:bg-[#005f69] flex items-center shadow-[4px_4px_0_0_rgba(17,17,17,1)] active:translate-y-1 active:shadow-none transition-all">
                        <Icons.Download /> <span className="ml-2">Exportar CSV</span>
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white border-4 border-black shadow-[6px_6px_0px_0px_#111111] overflow-x-auto print:shadow-none print:overflow-visible">
                <table className="w-full text-left border-collapse min-w-[800px] print:min-w-full">
                    ...
                    <tbody>
                        {items.map((b, i) => (
                            <tr key={i} className="border-b-2 border-gray-200 hover:bg-[#007D8A]/20 transition-colors page-break-inside-avoid">
...
const ListaInstituicoes = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in w-full max-w-6xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 no-print">
                <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter">Raio-X: Instituições</h2>
                <div className="flex flex-wrap gap-2">
                    <button onClick={() => window.print()} className="bg-black text-white px-4 py-2 font-black uppercase text-[10px] md:text-xs border-4 border-black hover:bg-gray-800 flex items-center shadow-[4px_4px_0_0_rgba(17,17,17,1)] active:translate-y-1 active:shadow-none transition-all">
                        <Icons.Printer /> <span className="ml-2">Exportar PDF</span>
                    </button>
                    <button onClick={() => exportToCSV(items, 'tabulum_instituicoes')} className="bg-[#C1272D] text-white px-4 py-2 font-black uppercase text-[10px] md:text-xs border-4 border-black hover:bg-[#a02025] flex items-center shadow-[4px_4px_0_0_rgba(17,17,17,1)] active:translate-y-1 active:shadow-none transition-all">
                        <Icons.Download /> <span className="ml-2">Exportar CSV</span>
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white border-4 border-black shadow-[6px_6px_0px_0px_#111111] overflow-x-auto print:shadow-none print:overflow-visible">
                <table className="w-full text-left border-collapse min-w-[800px] print:min-w-full">
                    ...
                    <tbody>
                        {items.map((m, i) => (
                            <tr key={i} onClick={() => setSelectedEntity({ type: 'instituicao', name: m.nome })} className={`border-b-2 border-gray-200 cursor-pointer transition-colors page-break-inside-avoid ${m.isPublic ? 'hover:bg-[#C1272D]/10' : 'hover:bg-[#EAA221]/10'}`}>
            ...
            {dadosAgregados.outros.qty > 0 && (
                <div className="p-4 bg-gray-100 border-2 border-dashed border-gray-400 flex justify-between items-center cursor-pointer hover:bg-gray-200 transition-colors shadow-sm page-break-inside-avoid" onClick={() => setSelectedEntity({ type: 'instituicao', name: 'Entidades Não Informadas' })}>
...
const FichaCompleta = () => {
    ...
    return (
        <div className="space-y-6 w-full max-w-6xl mx-auto pb-12 animate-fade-in">
            ...
            <div className="bg-white p-6 md:p-8 border-4 border-black shadow-[8px_8px_0_0_rgba(17,17,17,1)] relative overflow-hidden print:shadow-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                 <div className="bg-white border-4 border-black shadow-[6px_6px_0_0_rgba(17,17,17,1)] flex flex-col lg:col-span-2 print:shadow-none print:break-inside-avoid">
                    ...
                    <div className="overflow-y-auto max-h-[350px] p-2 space-y-2 custom-scrollbar print:max-h-none">
                ...
                <div className="bg-white border-4 border-black shadow-[6px_6px_0_0_rgba(17,17,17,1)] flex flex-col lg:col-span-2 print:shadow-none print:break-inside-avoid">
                    <div className="p-4 bg-[#EAA221] border-b-4 border-black flex justify-between items-center text-black">
    ...
    return (
        <div className="space-y-4 md:space-y-6 mb-6">
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
                <div className={`col-span-2 lg:col-span-1 border-4 border-black p-3 sm:p-4 shadow-[4px_4px_0_0_rgba(17,17,17,1)] flex flex-col justify-between print:shadow-none ${isFiltroCapital ? 'bg-[#007D8A] text-white' : 'bg-[#EAA221] text-black'}`}>
                ...
                <div className="bg-white border-4 border-black p-3 sm:p-4 shadow-[4px_4px_0_0_rgba(17,17,17,1)] flex flex-col justify-between print:shadow-none">
                ...
                <div className="bg-white border-4 border-black p-3 sm:p-4 shadow-[4px_4px_0_0_rgba(17,17,17,1)] flex flex-col justify-between overflow-hidden print:shadow-none">
...
const AppContent = () => {
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen w-full overflow-hidden bg-[#FDFBF7] text-[#111111] font-sans relative print:h-auto print:overflow-visible">
            <Sidebar />
            <main className="flex-1 flex flex-col overflow-hidden relative print:overflow-visible">
                <div className="absolute inset-0 opacity-[0.04] pointer-events-none print:hidden" style={{ backgroundImage: 'radial-gradient(#111 1.5px, transparent 1.5px)', backgroundSize: '24px 24px' }}></div>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 relative z-10 custom-scrollbar print:overflow-visible">
            ...
            <style dangerouslySetInnerHTML={{__html: `
                ...
                * { scrollbar-color: #111 #FDFBF7; }
                ...
                @media print {
                    ...
                    .overflow-y-auto, .overflow-x-auto, .custom-scrollbar { overflow: visible !important; max-height: none !important; }
                    .shadow-\\[6px_6px_0_0_rgba\\(17\\,17\\,17\\,1\\)\\], 
                    .shadow-\\[8px_8px_0_0_rgba\\(17\\,17\\,17\\,1\\)\\], 
                    .shadow-\\[4px_4px_0_0_rgba\\(17\\,17\\,17\\,1\\)\\] { 
